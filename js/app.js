...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  let sums = sum(sum(a, b)[0], c)[0];
  let product = multiply(multiply(a, b)[0], c)[0];
  let messageOne = `${a} and ${b} and ${c} sum to ${sums}.`;
  let messageTwo = `The product of ${a} and ${b} and ${c} is ${product}.`;
  // console.log([sum, product, messageOne, messageTwo]);
  return [sums, product, messageOne, messageTwo];
}
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  for (let i = 0; i < sumArr.length; i++) {
    ...
    sums = sum(sums, currentNumber)[0];
    if (i !== sumArr.length - 1) { // if current number is not last element in the array
      message += `${currentNumber},`;
    } else { // if current number is last element in the array
      message += `${currentNumber} `;
    }
  }
  message += `was passed in as an array of numbers, and ${sums} is their sum.`;
...
function multiplyArray(multArr) { //eslint-disable-line
  ...
  for (let i = 0; i < multArr.length; i++) {
    let currentNumber = multArr[i];
    product = multiply(product, currentNumber)[0];
    if (i !== multArr.length - 1) { // if current number is not the last element in the array
      message += `${currentNumber},`;
